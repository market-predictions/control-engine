...
PROTOCOL_ID = "CONTROL_V4_RUNTIME_CARRIER_V1"
...
def strict_json_object(text: str) -> dict[str, Any]:
    try:
        ...
        raise RuntimeProtocolError("command JSON invalid or ambiguous") from exc
    if not isinstance(value, dict):
        raise RuntimeProtocolError("command payload must be an object")
    return value
...
def _task_id(value: object) -> str:
    if not isinstance(value, str) or not value or len(value) > 300 or any(c in value for c in "\r\n\t"):
        raise RuntimeProtocolError("task_id invalid")
    return value
...
def parse_public_command(comment_body: str) -> dict[str, Any]:
    ...
    if comment_body.startswith(TICK_PREFIX):
        ...
        _exact_keys(payload, {"run_id", "yielded_task_ids"}, {"run_id"})
        payload["run_id"] = _run_id(payload["run_id"])
        yielded = payload.get("yielded_task_ids", [])
        if not isinstance(yielded, list) or len(yielded) > 100:
            raise RuntimeProtocolError("yielded_task_ids invalid")
        clean: list[str] = []
        for item in yielded:
            task = _task_id(item)
            if task in clean:
                raise RuntimeProtocolError("yielded task duplicated")
            clean.append(task)
        return {"kind": "TICK", "run_id": payload["run_id"], "yielded_task_ids": clean}
    ...
    if comment_body.startswith(EVENT_PREFIX):
        ...
        required = {"run_id", "task_id", "event"} | event_fields[event]
        allowed = common | event_fields[event]
        _exact_keys(payload, allowed, required)
        payload["run_id"] = _run_id(payload["run_id"])
        payload["task_id"] = _task_id(payload["task_id"])
...
def validate_runtime_binding(
    ...
    *,
    ...
    if authority.get("principal_manual_relay_count") != 0:
        raise RuntimeProtocolError("runtime authority relay count invalid")
    ...
    if runner_config.get("principal_manual_relay_count") != 0:
        raise RuntimeProtocolError("runner relay count invalid")
    ...
    if not isinstance(prompt_text, str) or any(marker not in prompt_text for marker in required_prompt_markers):
...
def assert_event_identity(queue: Mapping[str, Any], command: Mapping[str, Any], *, now: datetime) -> Mapping[str, Any]:
    ...
    if lock.get("task_id") != task_id or lock.get("run_id") != run_id:
        raise StaleEventError("event holder identity is stale")
    if _parse_ts(lock.get("expires_at")) <= _now(now):
        raise StaleEventError("event holder lease expired")
    ...
    elif any(value is not None for value in holder_fields):
        raise StaleEventError("event supplied candidate identity for candidate-less task")
    return task
...
def recover_expired_lock_v4(queue: Mapping[str, Any], *, now: datetime) -> tuple[dict[str, Any], bool]:
    ...
    if lock is None:
        return q, False
    if _parse_ts(lock["expires_at"]) > _now(now):
        return q, False
    ...
    *,
    ...
    if integration_enabled is True:
        for task in queue["tasks"]:
            if task["task_id"] in yielded:
                continue
            if task["status"] == "READY" and task["integration_policy"] == "AUTO_AFTER_PASS":
                return task["task_id"]

    for task in queue["tasks"]:
        if task["task_id"] not in yielded and task["status"] == "QUEUED":
            return task["task_id"]
    return None
...
def reconcile_review_candidate_drift_v4(
    ...
    *,
    ...
    task = assert_event_identity(
        queue,
        {"task_id": task_id, "run_id": run_id,
         "holder_candidate_sha": _task(queue, task_id).get("candidate", {}).get("candidate_sha") if isinstance(_task(queue, task_id).get("candidate"), Mapping) else None,
         "holder_expected_base_branch": _task(queue, task_id).get("candidate", {}).get("expected_base_branch") if isinstance(_task(queue, task_id).get("candidate"), Mapping) else None,
         "holder_expected_base_sha": _task(queue, task_id).get("candidate", {}).get("expected_base_sha") if isinstance(_task(queue, task_id).get("candidate"), Mapping) else None},
        now=now,
    )
    ...
    exact = (
        ...
        and live_candidate.get("expected_base_sha") == candidate.get("expected_base_sha")
    )
    if exact:
        return deepcopy(queue), False
    q = deepcopy(queue)
    ...
    changed["updated_at"] = _now(now).replace(microsecond=0).isoformat().replace("+00:00", "Z")
    validate_queue_v4(q)
    return q, True
...
def candidate_ready_v4(
    ...
    *,
    ...
    if task.get("status") != "ACTIVE" or task.get("phase") not in {"BUILD", "REPAIR"}:
        raise RuntimeProtocolError("candidate-ready event requires ACTIVE BUILD/REPAIR")
    expected = {
    ...
    if dict(verified_candidate) != expected:
        raise StaleEventError("candidate-ready live PR identity mismatch")
    q = deepcopy(queue)
...
def _review_matches(task: Mapping[str, Any]) -> bool:
    candidate = task.get("candidate")
...
def internal_review_v4(
    ...
    *,
    ...
    task = assert_event_identity(queue, command, now=now)
    if task.get("status") != "ACTIVE" or task.get("phase") != "REVIEW":
        raise RuntimeProtocolError("internal review event requires ACTIVE/REVIEW")
    ...
    if verdict not in {"PASS", "REPAIR_REQUIRED"}:
        raise RuntimeProtocolError("internal review verdict invalid")
    ...
    changed["last_review"] = {
        ...
        "expected_base_sha": candidate["expected_base_sha"],
        "verdict": verdict,
        "reviewed_at": _now(now).replace(microsecond=0).isoformat().replace("+00:00", "Z"),
    }
...
def external_requested_v4(queue: Mapping[str, Any], command: Mapping[str, Any], *, now: datetime) -> dict[str, Any]:
    ...
    if task.get("status") != "ACTIVE" or task.get("phase") != "REVIEW" or task.get("review_policy") != "EXTERNAL":
        raise RuntimeProtocolError("external-request event requires EXTERNAL ACTIVE/REVIEW")
    if not _review_matches(task):
        raise RuntimeProtocolError("external request requires exact internal PASS")
    candidate = task["candidate"]
    q = deepcopy(queue)
    changed = next(item for item in q["tasks"] if item["task_id"] == command["task_id"])
    existing = changed.get("external_review")
    if isinstance(existing, Mapping) and existing.get("request_ref") not in {None, command["request_ref"]}:
        raise StaleEventError("external review request already bound differently")
...
def external_unavailable_v4(queue: Mapping[str, Any], command: Mapping[str, Any], *, now: datetime) -> dict[str, Any]:
    task = assert_event_identity(queue, command, now=now)
    if task.get("status") != "ACTIVE" or task.get("phase") != "REVIEW" or task.get("review_policy") != "EXTERNAL":
        raise RuntimeProtocolError("review-unavailable event requires EXTERNAL ACTIVE/REVIEW")
    candidate = task.get("candidate")
    if not isinstance(candidate, Mapping):
        raise RuntimeProtocolError("review-unavailable event requires candidate")
    q = deepcopy(queue)
    changed = next(item for item in q["tasks"] if item["task_id"] == command["task_id"])
    existing = changed.get("external_review")
    changed["external_review"] = {
...
def external_finding_v4(queue: Mapping[str, Any], command: Mapping[str, Any], *, now: datetime) -> dict[str, Any]:
    task = assert_event_identity(queue, command, now=now)
    if task.get("status") != "ACTIVE" or task.get("phase") != "REVIEW" or task.get("review_policy") != "EXTERNAL":
        raise RuntimeProtocolError("external-finding event requires EXTERNAL ACTIVE/REVIEW")
    candidate = task.get("candidate")
    if not isinstance(candidate, Mapping):
        raise RuntimeProtocolError("external-finding event requires candidate")
...
def external_pass_v4(
    ...
    *,
    ...
    task = assert_event_identity(queue, command, now=now)
    if task.get("status") != "ACTIVE" or task.get("phase") != "REVIEW" or task.get("review_policy") != "EXTERNAL":
        raise RuntimeProtocolError("external-pass event requires EXTERNAL ACTIVE/REVIEW")
    if not _review_matches(task):
        raise RuntimeProtocolError("external PASS requires exact internal PASS")
    candidate = task["candidate"]
    existing = task.get("external_review")
    q = deepcopy(queue)
    changed = next(item for item in q["tasks"] if item["task_id"] == command["task_id"])
...
def safe_work_capsule(
    ...
    *,
    ...
    action = phase
    if phase == "REVIEW":
        if task.get("review_policy") == "INTERNAL" or not _review_matches(task):
            action = "REVIEW_INTERNAL"
        else:
            external = task.get("external_review")
            if not isinstance(external, Mapping) or external.get("request_ref") is None:
                action = "REQUEST_EXTERNAL_REVIEW"
            else:
                action = "RECONCILE_EXTERNAL_REVIEW"
