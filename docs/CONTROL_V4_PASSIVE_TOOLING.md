...
- a bounded V3.1 -> V4 transform wrapper that consumes the exact committed V4 authority blobs, fails closed unless the fenced source has zero live claims and the reviewed queued-implementation shape, and preserves migration facts only after the existing canonical `migration_v31.validate_migration_facts()` semantics accept them;
