...
A Scheduled invocation must not obtain acquisition authority merely because it contains text that resembles the Runner prompt. Before the invocation posts its first TICK, the canonical Runner prompt requires one read-only scheduler readback to prove, simultaneously, the exact reviewed automation object, enabled state, exact hourly `:30` Europe/Amsterdam schedule with `timing_mode=exact_schedule`, current generation, canonical prompt identity markers, and absence of a second enabled Control V4 Runner. Failure means zero public command writes.

The current candidate command generation is `bdabf8391bbd1a6c`. Failed compact generation `83ee437c017961ce`, its predecessor `c81e7a4f2d1b9306`, and adopted predecessor `dcd5dd2495113a68` are historical/current-trust rejected once this candidate is adopted. This repair preserves compact scheduler deployability while restoring all three explicit fail-closed predicates found by fresh paired review: exact scheduler mode plus canonical prompt identity at pre-acquisition; immediate target-effect start after second-TICK revalidation with only the minimum fresh target-identity read in between; and independent current-generation plus immutable `created_at` `0..120` second admission before any private capability is created. It changes no semantic authority, queue topology, fixed lease, or atomic EVENT holder-boundary behavior. A generation is not a reusable descriptive label: any canonical Runner prompt change that can alter command authority, acquisition, command correlation, holder-closeout or target-effect behavior requires a new previously unused generation before adoption.
