...
`ENGINE_MANIFEST.json` is the component-local manifest, and `semantic_runtime_authority=false` means the public engine does not own Control semantics. It is never a source for current **global Control runtime status** and does **not** mean that the canonical Control V4 Runner is inactive. Current global status must be reconstructed from current private V4 authority and `control-plane@control-runtime-state:control/DISPATCH_QUEUE.json`, with bounded target evidence as needed.
