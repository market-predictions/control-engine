...
The current candidate command generation is `83ee437c017961ce`. Failed compact generation `c81e7a4f2d1b9306` and adopted predecessor `dcd5dd2495113a68` are historical/current-trust rejected once this candidate is adopted. This repair preserves compact scheduler deployability while restoring the two explicit fail-closed predicates found by fresh paired review: exact scheduler mode plus canonical prompt identity at pre-acquisition, and immediate target-effect start after second-TICK revalidation with only the minimum fresh target-identity read in between. It changes no semantic authority, queue topology, fixed lease, or atomic EVENT holder-boundary behavior. A generation is not a reusable descriptive label: any canonical Runner prompt change that can alter command authority, acquisition, command correlation, holder-closeout or target-effect behavior requires a new previously unused generation before adoption.
