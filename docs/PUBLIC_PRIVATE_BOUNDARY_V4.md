...
`ENGINE_MANIFEST.json` is the component-local manifest, and `semantic_runtime_authority=false` means the public engine does not own Control semantics. It is never a source for current **global Control runtime status**. Current global status must be reconstructed from current private V4 authority and the canonical private queue, with bounded target evidence as needed.
