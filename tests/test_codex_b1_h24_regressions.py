...
def test_prior_review_at_current_request_second_is_ambiguous_not_clean_pass():
    result = _classify(
        issue_comments=[
    ...
    assert "timestamp boundary" in result.summary


def test_prior_finding_at_current_request_second_is_ambiguous_not_clean_pass():
    result = _classify(
        issue_comments=[
            _request(100, PRIOR_HANDOVER, PRIOR_AT),
            _request(200, CURRENT_HANDOVER, CURRENT_AT),
...
def test_review_at_next_request_second_is_ambiguous_for_current_request():
    result = _classify(
        issue_comments=[
            _request(200, CURRENT_HANDOVER, CURRENT_AT),
...
def test_first_request_can_accept_same_second_exact_review_when_no_adjacent_boundary_exists():
    result = _classify(
