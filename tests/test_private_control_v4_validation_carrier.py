...
def test_objective_auto_accept_is_internal_only_and_fail_closed_on_uncertainty():
    markers = validator.OBJECTIVE_AUTO_ACCEPT_PROMPT_REQUIRED_MARKERS
    assert "objective_auto_accept_policy=OBJECTIVE_EVIDENCE_V1" in markers
    assert any("INTERNAL review only" in marker for marker in markers)
    assert any("binary evidence" in marker for marker in markers)
    assert any("EXTERNAL" in marker and "explicit external exact-candidate PASS" in marker for marker in markers)
    assert any("auto-accept is forbidden" in marker for marker in markers)
    assert any("never means auto-merge" in marker for marker in markers)
