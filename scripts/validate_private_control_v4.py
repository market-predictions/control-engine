...
def validate_current_surface(root: Path, entries: Mapping[str, tuple[str, str, str]]) -> None:
...
def validate_runtime_and_runner(root: Path, entries: Mapping[str, tuple[str, str, str]]) -> dict[str, Any]:
    ...
    for marker in (
        "status=ACTIVE_BOUND",
        "reconcile correlated live GitHub external-review evidence",
...
def validate_system_index(raw: bytes, runtime: Mapping[str, Any]) -> None:
    ...
    required = {
        ...
        "Current human status is an ephemeral projection",
